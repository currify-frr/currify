...
function limparTudo() {
  ...
  alert("Resumo gerado com base nas suas informações. Ajuste se desejar.");
  atualizarPreview();
}
...
const btnUsarModelo = document.getElementById("usarModelo");

if (btnUsarModelo) {
  btnUsarModelo.addEventListener("click", () => {
    document.body.setAttribute("data-template", modeloSelecionado);
    document.getElementById("templateOverlay").style.display = "none";
    document.body.classList.remove("no-scroll");
    localStorage.setItem("template", modeloSelecionado);
  });
}


const salvo = localStorage.getItem("template");
if (salvo) {
  document.body.setAttribute("data-template", salvo);
}
